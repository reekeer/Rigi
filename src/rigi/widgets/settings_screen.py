...
class _CategorySelected(Message):
...
class _CategoryRow(Widget):
    ...
    def on_click(self) -> None:
        self.post_message(_CategorySelected(self._cat_name))
...
class _SettingInput(Input):
    """Editable input for a setting with write_fn or stored value."""
...
class RigiSettingsScreen(ModalScreen[None]):
    ...
    def on_mount(self) -> None:
        outer = self.query_one("#s-outer")
        outer.border_title = "⚙  Settings"
        self._render_category(self._active_category)

    def on__category_selected(self, event: _CategorySelected) -> None:
        event.stop()
